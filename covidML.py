...
def main():
    ...
    factors = [
        "GDP_PER_CAPITA",
        "POPULATION_DENSITY",
    ]  # not sure how this will work, but I would like these to contain the headers of the data set or some other way of referencing what we are using as factors
    degree = 3
    ...
    def genRan():  # sets intial weight to random vaule between -1 and 1
        return (random.random() - 0.5) * 2  # fix
    ...
    toolbox.register(
        ...
        creator.Individual,
        toolbox.Alpha,
        n=(degree) * len(factors),
    )  # sets up weights in an indviaul based on the degree of the formuala and the number of factors
    ...
    def predictor(
        individual, ISO
    ):  # this function should take an indviudal and return the predicted value for a given set of days
        ...
        for i in range(1, T):
            ...
            for j in range(len(factors)):
                for k in range(degree):
                    summ += (
                        individual[(j * degree) + k]
                        * math.pow(i, k + 1)
                        * (df.loc[ISO].head(1)[factors[j]] / max_vals[factors[j]])
                    )
    ...
    def eval(
        individual,
    ):  # this function will be our eval function, aka the fitness function, the closer to zero the better
        tester = 0
        for i in codes:
    ...
    toolbox.register(
        "evaluate", eval
    )  # lets the code recgognize what our eval/fitness function is
    toolbox.register(
        "mate", tools.cxBlend, alpha=0.05
    )  # sets up mate to produce a new individual with blended weights
    toolbox.register(
        "mutate", tools.mutPolynomialBounded, eta=0.2, low=low1, up=up1, indpb=0.5
    )  # sets up mutation chance during mate procces
    toolbox.register(
        "select", tools.selTournament, tournsize=PopS
    )  # Seems best to have this match popsize
    # Above code block sets up info need for ml procces
    print("ML Proccess started")

    for gen in range(NGEN):
        offspring = algorithms.varAnd(
            population, toolbox, cxpb=0.5, mutpb=0.25
        )  # cxpd is mating chance and mutpb is the chance of mutation
        fits = toolbox.map(toolbox.evaluate, offspring)
        for fit, ind in zip(fits, offspring):
            ind.fitness.values = fit
        top = tools.selBest(
            population, k=1
        )  # saves the best of each gen to top, no use now but may be useful for internal stats later
        fts = "\t"
        for j in range(len(factors)):
            fts += factors[j] + ": \t"
            for k in range(degree):
